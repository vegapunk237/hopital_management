...
from .models import Medecin, Prescription, Consultation, Disponibilite
...
class DisponibiliteListAPIView(generics.ListAPIView):
    queryset = Disponibilite.objects.all()
    serializer_class = DisponibiliteSerializer
    
class AjoutDisponibilite(View):
    templates_name = 'personnel/ajout_disponibilite.html'
    ...
    def post(self, request, *args, **kwags):
        ...
        data = {
            'medecin': request.POST.get('name'),
            'date': request.POST.get('date'),
            'heure': request.POST.get('time')

        }
        
        created = Disponibilite.objects.create(**data)

        if created:

            print("disponibilite creer avec success")
        else:

            print(" la disponibilite n'as pas ete  creer ")
...
def get_disponibilite_list():
    try:
        ...
        response = requests.get('http://patientso:8003/api/rdv/')
        if response.status_code == 200:
...
def create_rdv(request):
    ...
    return render(request, 'personnel/ajout_disponibilite.html', {'patient_list': patient_list})
